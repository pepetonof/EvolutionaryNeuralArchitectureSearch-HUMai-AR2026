...
class Dataset2D(Dataset):
    ...
    def __getitem__(self, index:int):
        ...
        image=io.imread(img_id)
        mask =io.imread(mask_id)
        # mask = io.imread(mask_id, as_gray=True)
        # mask = np.asarray(mask, dtype=np.uint8)    
        mask = np.squeeze(mask)
        # print('IMG-Readed', img_id, image.shape, image.max(), image.min(), image.dtype)
        # print('MASK-Readed', mask_id, mask.shape, mask.max(), mask.min(), mask.dtype, np.unique(mask))
        ...
        if self.transform is not None:
            # print("img", img_id, image.shape)
            # print("mask", mask_id, mask.shape)
            augmentations = self.transform(image=image, mask=mask)
            image=augmentations["image"]
            mask=augmentations["mask"]
        
        # print('IMAGET\t', img_id, index, image.shape, image.max(), image.min(), image.dtype)
        # print('MASKT\t', mask_id, index, mask.shape, mask.max(), mask.min(), mask.dtype, np.unique(mask)) 
        
        return image, mask
    
# def sorting_images(_id, )