...
# from toolbox_functions import identifier
...
def get_data(p, val_size=0.1, _format='.png'):
    ...
    #polyps
    TRAIN_IMG_DIR.sort()
    TRAIN_MASK_DIR.sort(key=lambda img_id: img_id.name.replace("_mask", "")) #solo para polyps
    TEST_IMG_DIR.sort()
    TEST_MASK_DIR.sort(key=lambda img_id: img_id.name.replace("_mask", "")) #solo para polyps
    ...
    if 'valid' in [d.name for d in dirs]:
        ...
        #solo para polyps
        VAL_IMG_DIR.sort()
        VAL_MASK_DIR.sort(key=lambda img_id: img_id.name.replace("_mask", "")) #solo para polyps
...
# train, valid, test = get_data(path_images, _format='.png') #train, valid, test = 
# print(len(train))
# print(len(test))
# print(len(valid))
