...
class PrimitiveTree(list):
    ...
    def from_string(cls, string, pset):
        """
        Convert a string representation of an individual (GP expression)
        into a PrimitiveTree, compatible with COGP / NASGP relaxations.
    
        - Accepts sibling or cross-layer types (no hard TypeError)
        - Handles MetaEphemeral and numeric terminals
        - Compatible with DEAP 1.4.3 and NumPy 2.x
        """
        ...
        for token in tokens:
            ...
            # --- Known primitive or terminal name ---
            if token in pset.mapping:
                prim = pset.mapping[token]
    
                # RELAXED type validation
                if type_ is not None and not (
                    issubclass(prim.ret, type_) #or issubclass(type_, prim.ret)
                ):
                    warnings.warn(
                        f"[COGP relaxed parse] {prim.name} returns {prim.ret.__name__}, "
                        f"expected {type_.__name__}; continuing."
                    )
    
                expr.append(prim)
    
                # If primitive, enqueue its argument types (for STGP order)
                if isinstance(prim, Primitive):
                    ret_types.extendleft(reversed(prim.args))
    
            else:
                # --- Numeric or symbolic literal ---
                try:
                    value = eval(token)
                except Exception:
                    value = token  # keep string if eval fails
    
                if type_ is None:
                    type_ = type(value)
    
                # Instantiate MetaEphemeral or standard class terminals
                if isclass(type_) or isinstance(type_, MetaEphemeral):
                    term = Terminal(value, False, type_)
                else:
                    # Relaxed terminal type check
                    if not (
                        issubclass(type(value), type_) #or issubclass(type_, type(value))
                    ):
                        warnings.warn(
                            f"[COGP relaxed parse] terminal {value} type "
                            f"{type(value).__name__} ≠ expected {type_.__name__}; coercing."
                        )
                    term = Terminal(value, False, type_)
    ...
    #             #RELAXED TYPE VALIDATION
    #             if type_ is not None and not (
    #                     issubclass(primitive.ret, type_) #or issubclass(type_, primitive.ret)
    #             ):
    #                 warnings.warn(
    #                     f"[COGP relaxed parse] {primitive.name} returns "
    #                     f"{primitive.ret.__name__}, expected {type_.__name__}; continuing."
    #                 )
    ...
    #             # Instantiate MetaEphemeral or standard class terminals
    #             if isclass(type_) or isinstance(type_, MetaEphemeral):
    #                 term = Terminal(token, False, type_)
    ...
    #                 # # Relaxed terminal type check
    #                 # if not (issubclass(type(token), type_) #or issubclass(type_, type(token))
    #                 # ):
    #                 #     warnings.warn(
    #                 #         f"[COGP relaxed parse] terminal {token} type "
    #                 #         f"{type(token).__name__} ≠ expected {type_.__name__}; coercing."
    #                 #     )
    #                 #     term = Terminal(token, False, type_)
...
def graph(expr):
    ...
    for i, node in enumerate(expr):
        # print(len(stack), stack, i, node.arity)
        if stack:
            ...
            try:
                ...
                # No terminal fits, either - that's an error
                    ...
                    raise IndexError("The gp.generate function tried to add " \
                                      "a terminal of type '%s', but there is " \
                                      "none available." % (type_,), traceback)
                if isclass(term) or isinstance(term, MetaEphemeral):
                # if type(term) is MetaEphemeral:
                    term = term()
                expr.append(term)
    #print(len(expr))
...
def generateMD(pset, min_, max_, condition, type_=__type__):
    if type_ is None:
        type_ = pset.ret
    expr = []
    height = random.randint(min_, max_)
