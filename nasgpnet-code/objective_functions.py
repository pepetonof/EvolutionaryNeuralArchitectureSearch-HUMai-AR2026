...
#Version of MO: logarithms of parameters
def evaluate_complexity(model):
    """Make model"""
    # model = make_model(ind, in_channels, pset)
    #params=sum(p.numel() for p in model.parameters() if p.requires_grad)
    
    return sum(p.numel() for p in model.parameters() if p.requires_grad)#np.log(params)#(max_params - params)/max_params, params
...
# def dice_loss_surrogate(ind, surrogate_model, 
#                          in_channels,
#                          out_channels,
#                          pset,
#                          complexity):
